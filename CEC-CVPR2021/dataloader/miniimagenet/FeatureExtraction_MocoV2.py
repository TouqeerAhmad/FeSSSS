...
def getTrainSet(session_ID, model_Type_input):
    ...
    txt_path = "../../data/index_list/mini_imagenet/" + session_file_names[session_ID-1]
    ...
    batch_size_base = 400
    if (session_ID-1) == 0:
        trainset = MiniImageNet(root=dataroot, train=True, transform=None, index_path=txt_path, index=class_index, base_sess=True, modelType=model_Type_input) # for base session
    else:
        trainset = MiniImageNet(root=dataroot, train=True, transform=None, index_path=txt_path, base_sess=False, modelType=model_Type_input)
...
def getValSet(session_ID, model_Type_input):
    ...
    batch_size_base = 400
    if (session_ID-1) == 0:
        trainset = MiniImageNet(root=dataroot, train=False, transform=None, index=class_index, modelType=model_Type_input)
    else:
        trainset = MiniImageNet(root=dataroot, train=False, transform=None, index=class_index, modelType=model_Type_input)
    cls = np.unique(trainset.targets)
    print(trainset.__len__())
