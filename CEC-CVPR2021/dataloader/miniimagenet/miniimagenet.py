...
class MiniImageNet(Dataset):
    ...
    def __init__(self, root='./data', train=True,
                 transform=None,
                 index_path=None, index=None, base_sess=None, modelType=None):
        if train:
            setname = 'train'
        else:
            setname = 'test'
        ...
        if train:
            ...
            self.transform = transforms.Compose([
                ...
                transforms.Resize(224), # tentatively
...
if __name__ == '__main__':
    ...
    batch_size_base = 400
    trainset = MiniImageNet(root=dataroot, train=True, transform=None, index_path=txt_path, index=class_index, base_sess=True)
    ...
    batch_size_base = 400
    trainset = MiniImageNet(root=dataroot, train=True, transform=None, index_path=txt_path, base_sess=False)
    cls = np.unique(trainset.targets)
    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size_base, shuffle=True, num_workers=8,
                                              pin_memory=True)
    print(cls)
    ...
    txt_path = "../../data/index_list/mini_imagenet/session_3.txt"
    #class_index = open(txt_path).read().splitlines()
    base_class = 70
    class_index = np.arange(base_class)
    #dataroot = '~/data'
    ...
    base_class = 60
    class_index = np.arange(base_class)
    valset = MiniImageNet(root=dataroot, train=False, transform=None, index=class_index)
    cls = np.unique(valset.targets)
    print(cls)
