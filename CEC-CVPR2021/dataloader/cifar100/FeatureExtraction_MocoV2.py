...
def extract_features(N, N_feature, val_loader, modelObj, special_net):
    print('N: ', N)
        ...
        for i, (images, target) in enumerate(val_loader, 0):
            ...
            for layer in layer_outputs:
                ...
                if special_net == 'cec':
                     ...
                     layer_outputs[layer] = m(layer_outputs[layer])
...
def getTrainSet(session_ID, model_Type_input):
    
    session_file_names = ['session_1.txt', 'session_2.txt', 'session_3.txt', 'session_4.txt', 'session_5.txt', 'session_6.txt', 'session_7.txt',
                            'session_8.txt', 'session_9.txt']
    txt_path = "../../data/index_list/cifar100/" + session_file_names[session_ID-1]
    
    num_classes_per_session = [60, 65, 70, 75, 80, 85, 90, 95, 100]
    base_class = num_classes_per_session[session_ID-1]
    class_index = np.arange(base_class)
    dataroot = '~/dataloader/data/'
    batch_size_base = 400
    if (session_ID-1) == 0:
        trainset = CIFAR100(root=dataroot, train=True, download=True, transform=None, index=class_index, base_sess=True) # for base session
    else:
        class_index = open(txt_path).read().splitlines()
        trainset = CIFAR100(root=dataroot, train=True, download=True, transform=None, index=class_index, base_sess=False)
...
def getValSet(session_ID, model_Type_input):
    ...
    if (session_ID-1) == 0:
        ...
        testset = CIFAR100(root=dataroot, train=False, download=False,index=class_index, base_sess=False)
    cls = np.unique(testset.targets)
...
def load_CEC_stat_dict(checkpoint_path):
    sys.path.insert(1, '/home/tahmad/work/FSCIL/CEC-CVPR2021/models')
    from resnet20_cifar import resnet20
    model = resnet20(num_classes=64)
    if checkpoint_path is not None:
        state_dict = torch.load(checkpoint_path, map_location="cpu")
        state_dict = state_dict['params']
        new_state_dict = {}
        for k in state_dict:
            new_state_dict[k.replace('module.encoder.', '')] = state_dict[k]
...
def main(args):
    ...
    if args.weights is not None:
        if args.special_net == 'cec':
            model = load_CEC_stat_dict(args.weights)
        else:    
    ...
    if args.split == 'train':
        print('Train Split')
        train_dataset = getTrainSet(args.session_ID, args.model_Type_input)
    else:
        print('Val Split')
        train_dataset = getValSet(args.session_ID, args.model_Type_input)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=False, num_workers=n_cpu)
    feature_train = extract_features(train_dataset.__len__(), args.N_feature, train_loader, modelObj, args.special_net)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_Type_input",
                        default='supervised',
                        help="Supervised of self-supervised feature extracto, crop size being controlled based on this.")
